        ...
        <div class="card-body">
            <div class="col-lg-12">
                <div class="row justify-content-between">
                    ...
                    <a href="{{ route('attendances.rosters.index') }}" style="height: 40px !important;" class="btn btn-outline-secondary mr-1">{{ __('label.back') }}</a>
                </div>
                <br>
                ...
                <div class="table-responsive">
                    <table class="table">
                        ...
                        <tr>
                            ...
                            <th width="15%">{{ __('label.date') }}</th>
                            <th width="20%">{{ __('label.shift') }}</th>
                            <th width="15%">{{ __('label.start') }}</th>
                            <th width="15%">{{ __('label.end') }}</th>
                            <th width="*">{{ __('label.description') }}</th>
                        </tr>
                        </thead>
                        <tbody>
                        @csrf
                        ...
                        @for($i = 1; $i<=$totalDay; $i++)
                            ...
                            <tr>
                                ...
                                <td>
                                    <x-form.select name="shift_id_[{{ $i }}]" :datas="$shifts" options="- {{ __('select-option.choose shift') }} -" value="{{ $rosters[$i - 1]->shift_id ?? $shift->id  }}"
