<form id="form-edit" method="POST" class="form-validation"
      action="{{ empty($family) ? route('recruitments.applicants.family.store', $id) : route('recruitments.applicants.family.update', $family->id) }}"
    ...
    <x-form.input label="No. KTP" name="identity_number" numeric maxlength="16" placeholder="No. KTP" value="{{ $family->identity_number ?? '' }}" />
