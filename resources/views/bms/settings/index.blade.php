                ...
                @if(!$buildings->isEmpty())
                    @foreach($buildings as $key => $r)
                        <tr>
                            <td>{{ $key+1 }}</td>
                            <td>{{ $r->name }}</td>
                            <td align="center"><a href="{{ route('bms.'.$arrMenu['target'].'.shifts', $r->id) }}" style="text-decoration: underline; font-weight: bold">{{ $totalShift[$r->id] ?? 0 }}</a></td>
                            ...
                            <td align="center">
                                <a href="{{ route('bms.'.$arrMenu['target'].'.show', $r->id) }}" class="btn btn-icon btn-info"><i data-feather="search"></i></a>
                            </td>
                        </tr>
                    @endforeach
                @else
