<form id="form-edit" method="POST" class="form-validation"
      action="{{ empty($work) ? route('employees.'.$arrMenu['target'].'.work.store', $id) : route('employees.'.$arrMenu['target'].'.work.update', $work->id) }}"
    ...
    <x-form.datepicker label="Tanggal Mulai" class="col-md-12" required name="start_date" value="{{ $work->start_date ?? '' }}" />
    <x-form.datepicker label="Tanggal Selesai" class="col-md-12" name="end_date" value="{{ $work->end_date ?? '' }}" />
