    ...
    <div class="card">
        <div class="card-header justify-content-between">
            ...
            @if(isset($access['create']))
                <a href="{{ route('employees.education.create') }}" class="btn btn-primary"><i data-feather='plus'></i> Tambah Pendidikan</a>
            @endif
        </div>
                ...
                @if(!$education->isEmpty())
                    @foreach($education as $key => $r)
                        <tr>
                            ...
                            <td>{{ $r->level_id ? $masters['SPD'][$r->level_id] : '' }}</td>
                            <td>{{ $r->major }}</td>
                            <td align="center">
                                @if($r->filename)
                            ...
                            <td align="center">
                                @if(isset($access['edit']))
                                    <a href="{{ route('employees.education.edit', $r->id) }}" class="btn btn-icon btn-primary"><i data-feather="edit"></i></a>
                                @endif
                                @if(isset($access['destroy']))
                                    <button href="{{ route('employees.education.destroy', $r->id) }}" id="delete" class="btn btn-icon btn-danger">
