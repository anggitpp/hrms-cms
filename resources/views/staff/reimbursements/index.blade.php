    ...
    <div class="card">
        <div class="card-header justify-content-between">
            ...
            @if(isset($access['create']))
                <a href="{{ route('staff.reimbursements.create') }}" class="btn btn-primary"><i data-feather='plus'></i> Tambah Pengajuan</a>
            @endif
        </div>
                ...
                @if(!$reimbursements->isEmpty())
                    @foreach($reimbursements as $key => $r)
                        <tr>
                            ...
                            <td>{{ setCurrency($r->value) }}</td>
                            <td>
                                @if(isset($access['approve/edit']))
                                    <a href="{{ route('staff.reimbursements.approve.edit', $r->id) }}">
                                @endif
                            ...
                            <td align="center">
                                @if(isset($access['edit']) && $r->approved_status != 't')
                                    <a href="{{ route('staff.reimbursements.edit', $r->id) }}" class="btn btn-icon btn-primary"><i data-feather="edit"></i></a>
                                @endif
                                @if(isset($access['destroy']) && $r->approved_status != 't')
                                    <button href="{{ route('staff.reimbursements.destroy', $r->id) }}" id="delete" class="btn btn-icon btn-danger">
